...
namespace BMCP_GA
{
	...
	InitialPopulationWithoutRating::InitialPopulationWithoutRating(BMCP::Graph& graph, size_t size) :
		genotypeLenght(graph.slots),
		size(size)
	{
		genotypeStructure.resize(graph.nodes.size());
	...
	std::vector<GA::Specimen> InitialPopulationWithoutRating::generate()
	{
		std::vector<GA::Specimen> population;
		population.reserve(size);
